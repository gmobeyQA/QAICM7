# Module to Remove Virtual Hard Disks from the Virtual Machine
$ProgressTextBox.BackColor = "Black"
$ProgressTextBox.text=$ProgressTextBox.text+" (Remove Virtual Hard Disks from the Virtual Machine)"
...
$ProgressTextBox.text= "Additional disks already Removed from VM $CurrentHwVmName"
# a delay so the message can be seen
Start-sleep 5
}
else
...
$disk_Count+=1
$ProgressTextBox.text=""
$ProgressTextBox.text= $CurrentHwVmName+" '"+$HardDisk.Name+"' size is "+$HardDisk.CapacityGB+" GB, Provisioned as: "+$HardDisk.StorageFormat
# a delay so the message can be seen
Start-sleep 5
...
$ProgressTextBox.text=""
if ($disk_Count -eq 1)
{$ProgressTextBox.text= "$disk_Count disk Removed from VM $CurrentHwVmName"}
else {$ProgressTextBox.text= "$disk_Count disks Removed from VM $CurrentHwVmName"}
# a delay so the message can be seen
Start-sleep 5
}
...
$ProgressTextBox.text= "Lab 5 Task 2 Undone"
# End of Module