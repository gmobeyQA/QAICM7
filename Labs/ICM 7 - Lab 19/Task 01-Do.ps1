# MODULE TO Configure vSphere vMotion Networking on sa-esxi-01.vclass.local
...
if ($?)
{
 ...
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="$thisNic incorrectly added to $VMoStdvSwitch - Removing it"
  # a delay so the message can be seen
  Start-sleep 5
  ...
  $ProgressTextBox.text=""
  $ProgressTextBox.text="Will try to Add $VMoNic to $VMoStdvSwitch"
  # a delay so the message can be seen
  Start-sleep 5
  $ThisVMHostNetworkAdapter = (Get-VMhost $CurrentH1 | Get-VMHostNetworkAdapter -Physical -Name $VMoNic)
  Get-VirtualSwitch -VMHost $CurrentH1 -Name $VMoStdvSwitch| Add-VirtualSwitchPhysicalNetworkAdapter -VMHostPhysicalNic $ThisVMHostNetworkAdapter -Confirm:$false
...
$ProgressTextBox.text=""
$ProgressTextBox.text= "Lab 19 Task 1 Done"
# End of Module