# MODULE Undo Configure Network Management Redundancy
...
$vCenter=Connect-VIServer -Server $CurrentVC -User $CurrentVCUser -Password $CurrentVCPass # >$null 2>&1
 ...
 $ProgressTextBox.text="vMotion Kernel Port state will be unset"
 # a delay so the message can be seen
 Start-sleep 5
 ...
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="vMotion Kernel Port to be Disabled for Mangement on $CurrentH1 "
  Get-VMHost $CurrentH1 | Get-VMHostNetworkAdapter | where { $_.PortGroupName -eq $VMoVMPG } | Set-VMHostNetworkAdapter -ManagementTrafficEnabled $false -confirm:$false
  # a delay so the message can be seen
  Start-sleep 5
 }
 else
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="vMotion Kernel Port already Mangement Disabled on $CurrentH1 "
  # a delay so the message can be seen
  Start-sleep 5
 }
 ...
 if ($Thiskpt.ManagementTrafficEnabled)
 {
  ...
  Get-VMHost $CurrentH2 | Get-VMHostNetworkAdapter | where { $_.PortGroupName -eq $VMoVMPG } | Set-VMHostNetworkAdapter -ManagementTrafficEnabled $false -confirm:$false
  # a delay so the message can be seen
  Start-sleep 5
 }
 else
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="vMotion Kernel Port already Mangement Disabled on $CurrentH2 "
  # a delay so the message can be seen
  Start-sleep 5
 }
...
$ProgressTextBox.text=""
$ProgressTextBox.text= "Lab 26 Task 3 Undone"
# End of Module