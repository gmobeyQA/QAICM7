# MODULE Check Configure Network Management Redundancy
...
$vCenter=Connect-VIServer -Server $CurrentVC -User $CurrentVCUser -Password $CurrentVCPass # >$null 2>&1
 ...
 $ProgressTextBox.text="is the vMotion Kernel Port state set"
 # a delay so the message can be seen
 Start-sleep 5
 ...
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="vMotion Kernel Port Mangement Enabled on $CurrentH1 "
  # a delay so the message can be seen
  Start-sleep 5
  $labok=$true
  ...
  {
   $ProgressTextBox.text=""
   $ProgressTextBox.text="Redo Lab 26 task 3 vMotion Kernel Port Mangement disabled on on $CurrentH2 "
   # a delay so the message can be seen
   Start-sleep 5
   $labok=$false
  }
 ...
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="Redo Lab 26 task 3 vMotion Kernel Port Mangement disabled on $CurrentH1"
  # a delay so the message can be seen
  Start-sleep 5
 }
...
if ($LabOk)
{
 ...
 $ProgressTextBox.text= "Lab 26 Task 3 Completed"
}
else
{
 $ProgressTextBox.text=""
 $ProgressTextBox.BackColor="Red"
 $ProgressTextBox.text= "Redo Lab 26 Task 3"
}
# End of Module