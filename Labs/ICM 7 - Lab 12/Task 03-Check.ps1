...
if ($hostStore.SoftwareIScsiEnabled)
{
 ...
 $ProgressTextBox.text="iSCSI is enabled on $CurrentH2"
 # a delay so the message can be seen
  Start-Sleep 5
}
else
{
 $ProgressTextBox.text=""
 $ProgressTextBox.text="iSCSI is disabled on $CurrentH2"
  Start-Sleep 5
 $Labworked=$false
} 
...
if ($Labworked)
{
 ...
 $ProgressTextBox.text=""
 $ProgressTextBox.text= "Lab 12 Task 3 Correctly Done"
}
else
{
 $ProgressTextBox.BackColor="Red"
 $ProgressTextBox.text=""
 $ProgressTextBox.text= "Redo Lab 12 Task 3"
}
# End of Module