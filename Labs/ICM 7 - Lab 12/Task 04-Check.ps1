# MODULE Check Connect the iSCSI Software Adapters to Storage
...
if ("$ThisAdapter" -eq "")
{
 ...
 $ProgressTextBox.text="iSCSI Port Binding Not set"
 $Labworked=$false
}
else
{
 ...
 # a delay so the message can be seen
 Start-sleep 5
#Check the Discovery iSCSI IP addresses on the iSCSI Software Adapter
...
{
 $ProgressTextBox.text=""
 $ProgressTextBox.text="Send Targets Address is set to $IpStiSCSItgt"
 # a delay so the message can be seen
 Start-sleep 5
if ($thistgt.Port -eq "3260")
{
 $ProgressTextBox.text=""
 $ProgressTextBox.text="Send Targets Address is set to 3260"
 # a delay so the message can be seen
 Start-sleep 5
}
else
{
 $ProgressTextBox.text=""
 $ProgressTextBox.text="Send Targets Address is incorrectly set"
 # a delay so the message can be seen
 Start-sleep 5
 $Labworked=$false
}
...
if ($Labworked)
{
 ...
 $ProgressTextBox.text= "Lab 12 Task 4 Correctly Done"
}
else
{
 $ProgressTextBox.text=""
 $ProgressTextBox.BackColor="Red"
 $ProgressTextBox.text= "Redo Lab 12 Task 4"
}
# End of Module