...
$ProgressTextBox.text=$ProgressTextBox.text+" (Add the iSCSI Software Adapter to an ESXi Host)"
...
start-sleep 2
$Current_Host_Context = Get-VMHostStorage -VMHost $CurrentH2
#Enable iSCSI
...
else
{
 ...
 $Current_Host_Context = Get-VMHostStorage -VMHost $CurrentH2
 #Get-VMHostStorage -VMHost $CurrentH2 # for testing
 Get-VMHostStorage -VMHost $CurrentH2 | Set-VMHostStorage -SoftwareIScsiEnabled $true
 # a delay so the message can be seen
 Start-Sleep 5	
} 
...
$ProgressTextBox.text=""
$ProgressTextBox.text= "Lab 12 Task 3 Done"
# End of Module