# MODULE Check Create a Second VMFS Datastore
...
if ($?)
{
 ...
 $ProgressTextBox.text="Datastore $Current_ThirdDatatoreName has been Created"
 # a delay so the message can be seen
 Start-sleep 5
 $thisDatastore=Get-VMHost -Name $CurrentH1 | Get-Datastore -Name $Current_ThirdDatatoreName |Select Name,CapacityGB,
 ...
 if ($thisDatastore.LUN -eq $Curr_ThirdLun)
 {
  $ProgressTextBox.text=""
  $ProgressTextBox.text="Datastore $Current_ThirdDatatoreName Created with the correct Lun"
  # a delay so the message can be seen
  Start-sleep 5
 }
 else
 {
  $Labworked=$false
  $ProgressTextBox.text=""
  $ProgressTextBox.text="Datastore $Current_ThirdDatatoreName Created with an incorrect Lun"
  # a delay so the message can be seen
  Start-sleep 5
 }
}
else
{
 $Labworked=$false
 $ProgressTextBox.text=""
 $ProgressTextBox.text="Datastore $Current_ThirdDatatoreName is Missing or Misnamed"
 # a delay so the message can be seen
 Start-sleep 5
}
...
if ($Labworked)
{
 ...
 $ProgressTextBox.text= "Lab 13 Task 5 Correctly Done"
}
else
{
 $ProgressTextBox.text=""
 $ProgressTextBox.BackColor="Red"
 $ProgressTextBox.text= "Redo Lab 13 Task 5"
}
# End of Module
